    ...
    <template lang="html">
        <el-card class="box-card">
            <div>
                ...
                <el-table v-loading="loading" :data="list" highlight-current-row border size="small">
                        ...
                        <template slot-scope="scope">
                            <a :style="`margin-left: ${(scope.row.categoryLevel - 1) * 20}px`" class="category-row">
                                <i
                                    ...
                                    :class="[expandData[scope.row.id] ? 'el-icon-minus' : 'el-icon-plus']"
                                    @click="toggleExpend(scope.row)"
                                ></i>
                                <span v-if="scope.row.categoryLevel >= 3" style="margin-left: 20px"></span>
                                <span class="category-name"
                                    >{{ scope.row.categoryLevel }}&emsp;{{ scope.row.permissionName }}
                ...
                <!-- 编辑类目弹窗 -->
                <el-dialog title="编辑类目" :visible.sync="visible" width="30%">
                ...
                <!-- 新增类目弹窗 -->
                <el-dialog title="新增类目" :visible.sync="newVisible" width="50%">
                    <el-form label-width="100px">
                        <el-row>
                            <el-col :span="24">
                                <el-form-item label="权限名称">
                ...
                makeFlatData(list, level = 0) {
                    ...
                    for (; i < len; i += 1) {
                        ...
                        if (item.childList && this.expandData[item.id]) {
                            const childList = this.makeFlatData(item.childList, level + 1);
