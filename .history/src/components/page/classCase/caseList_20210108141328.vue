<template>
    <div>
        ...
        <div class="search">
            输入搜索：
            <div>
                <el-input label="登录名称" v-model="input" @blur="queryData" @input="queryData" placeholder="案例名称/ID"> </el-input>
            </div>
            <div>
    ...
    methods: {
        ...
        queryData() {
            ...
            if (this.input === '' || this.input == null) {
                ...
                console.log(this.tableData);
                const result = this.tableData.filter(writInfo => {
                    console.log(writInfo, '过滤信息');
                    return Object.keys(writInfo).some(function(key) {
                        console.log(key, '字段');
                        return (
                            String(writInfo[key])
                                .toLowerCase()
